...
from GUI.AddWord import AddWord
from database.WordManager import delete_word, get_words_list

script_directory = os.path.dirname(os.path.abspath(__file__))

class GameWords(QWidget):
    ...
    def init_words(self):
        ...
        self.words_array = get_words_list()
        if self.words_array:
            # Calculate the maximum width of the buttons based on the length of the longest word
            self.max_word_length = max(len(word) for word in self.words_array)
    ...
    def show_add_widget(self):
        custom_dialog = AddWord()
        custom_dialog.new_word_added.connect(self.add_new_word_to_list)
        custom_dialog.exec_()
    ...
    def create_button(self, label_text, style, width):
        ...
        button.word_clicked.connect(self.delete_word_from_list)  # Connect the word_clicked signal
        return button
    
    def delete_word_from_list(self, word):
        # Handle the deletion of the word from the database
        delete_word(word)
...
class HoverButton(QPushButton):
    ...
    def mousePressEvent(self, event):
        self.word_clicked.emit(self.text())
